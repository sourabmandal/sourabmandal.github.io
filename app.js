...
function loadLocalStorage() {
  // Synchronous — loads from localStorage instantly, no network wait
...
function syncRemoteStateBackground() {
  // Fire-and-forget: syncs Supabase in background without blocking anything
  loadRemoteState().then(remote => {
    ...
    renderPublications();
    updateIndexStats();
  }).catch(e => debugLog('syncRemoteStateBackground error', e));
}
...
async function fetchORCID() {
  ...
  const apiUrl = ORCID_API + '/' + ORCID_ID + '/works';
  console.log('[ORCID] Fetching:', apiUrl);

  try {
    const summaryRes = await fetch(apiUrl, { headers: { Accept: 'application/json' } });
    console.log('[ORCID] Status:', summaryRes.status, summaryRes.ok);

    if (!summaryRes.ok) throw new Error('HTTP ' + summaryRes.status);

    const summaryData = await summaryRes.json();
    console.log('[ORCID] Response keys:', Object.keys(summaryData));

    const groups = summaryData.group || [];
    console.log('[ORCID] Groups found:', groups.length);

    if (groups.length === 0) {
      list.innerHTML = '<div class="pub-loading" style="color:var(--muted)">No publications found on ORCID.</div>';
      setOrcidStatus('connected', '0 works');
      return;
    }

    allPublications = groups.map(function(g) {
      ...
      const extIds  = summary['external-ids'] && summary['external-ids']['external-id'] ? summary['external-ids']['external-id'] : [];
      const doiObj  = extIds.find(function(e) { return e['external-id-type'] === 'doi'; });
      const doi     = doiObj ? doiObj['external-id-value'] : '';
      return { putCode: putCode, type: type, year: year, title: title, journal: journal, url: url, doi: doi, authors: null };
    });

    allPublications.sort(function(a, b) { return (parseInt(b.year) || 0) - (parseInt(a.year) || 0); });

    console.log('[ORCID] Parsed', allPublications.length, 'publications. First:', allPublications[0]);
    ...
    console.error('[ORCID] FAILED:', err);
    list.innerHTML = '<div class="pub-loading" style="color:var(--red)">Could not load publications: <strong>' + err.message + '</strong><br><a href="https://orcid.org/' + ORCID_ID + '" target="_blank" style="color:var(--accent)">View on ORCID</a><br><span style="font-size:0.7rem;color:var(--muted)">Open F12 Console for details.</span></div>';
    setOrcidStatus('error', err.message);
  }
}
...
async function fetchAuthorsBackground() {
  // ORCID v3.0 bulk endpoint: GET /{orcid}/works/{putCode1},{putCode2},...
  // Comma-separated put-codes in the PATH (not query params). Max 100 per request.
  const putCodes = allPublications.map(function(p) { return p.putCode; });
  const BULK_SIZE = 100;

  for (var i = 0; i < putCodes.length; i += BULK_SIZE) {
    const chunk = putCodes.slice(i, i + BULK_SIZE);
    const bulkUrl = ORCID_API + '/' + ORCID_ID + '/works/' + chunk.join(',');
    console.log('[ORCID] Bulk author fetch:', bulkUrl);
    try {
      const res = await fetch(bulkUrl, { headers: { Accept: 'application/json' } });
      console.log('[ORCID] Bulk status:', res.status);
      if (!res.ok) { console.error('[ORCID] Bulk failed:', res.status, res.statusText); continue; }
      const data = await res.json();
      console.log('[ORCID] Bulk response keys:', Object.keys(data));
      const items = data.bulk || [];
      console.log('[ORCID] Bulk items:', items.length);

      items.forEach(function(item) {
        if (!item.work) return;
        const putCode = String(item.work['put-code']);
        const pub = allPublications.find(function(p) { return p.putCode === putCode; });
        if (!pub) return;
        const contributors = item.work.contributors && item.work.contributors.contributor ? item.work.contributors.contributor : [];
        ...
        const authorEl = document.getElementById('authors-' + putCode);
        if (authorEl) {
          if (pub.authors) {
      ...
      console.error('[ORCID] Bulk author error:', e);
    }
  }
}
...
//  FIX 4: Page renders instantly from localStorage; Supabase syncs in background
//  FIX 5: Author fetching is batched 5-at-a-time instead of sequential
// ═══════════════════════════════════════════════════════
function init() {
  ...
  // Step 1: Load from localStorage SYNCHRONOUSLY — instant, no network wait
  loadLocalStorage();
  renderActivitiesPublic();
  updateIndexStats();

  // Step 2: Restore admin session if previously logged in
  if (isAuthenticated()) {
    loggedIn = true;
    document.getElementById('login-btn').textContent = '⚙ Admin';
    document.getElementById('login-btn').classList.add('logged-in');
  ...
  // Step 3: Fire off both network calls in parallel — neither blocks the page
  fetchORCID();                  // fetches publications from ORCID
  syncRemoteStateBackground();   // syncs tags/IF/activities from Supabase
}
