...
async function fetchORCID() {
  ...
  try {
    // Step 1: Fetch the summary list to get all put-codes
    const summaryRes = await fetch(`${ORCID_API}/${ORCID_ID}/works`, {
      headers: { Accept: 'application/json' }
    });
    if (!summaryRes.ok) throw new Error('ORCID API ' + summaryRes.status);
    const summaryData = await summaryRes.json();

    const putCodes = (summaryData.group || []).map(g =>
      String(g['work-summary'][0]['put-code'])
    );
    ...
    for (let i = 0; i < putCodes.length; i += BULK_SIZE) {
      ...
      bulkResults.push(...(bulkData.bulk || []));
    }

    // Step 3: Parse each full work record
    allPublications = bulkResults
      .filter(item => item.work)
      .map(item => {
        const work    = item.work;
        const putCode = String(work['put-code']);
        const type    = (work.type || 'other').toLowerCase();
        ...
        const authors = contributors.length > 0
          ? contributors.map(c => c['credit-name']?.value || '').filter(Boolean).join(', ')
          : null;
        return { putCode, type, year, title, journal, url, doi, authors };
      });
...
// fetchAuthorsBackground is no longer needed — authors are fetched in the bulk call above.
// Kept as a no-op to avoid any potential reference errors.
function fetchAuthorsBackground() {}
