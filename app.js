...
async function fetchORCID() {
  ...
  try {
    // Step 1: Fetch summary list — this includes title, year, journal, doi.
    // The /works summary endpoint actually contains everything except contributors.
    const summaryRes = await fetch(`${ORCID_API}/${ORCID_ID}/works`, {
      headers: { Accept: 'application/json' }
    });
    if (!summaryRes.ok) throw new Error('ORCID summary API ' + summaryRes.status);
    const summaryData = await summaryRes.json();

    const groups = summaryData.group || [];

    if (groups.length === 0) {
      list.innerHTML = '<div class="pub-loading" style="color:var(--muted)">No publications found.</div>';
      setOrcidStatus('connected', '0 works');
      return;
    }

    // Step 2: Parse summaries immediately — render without authors first so user sees content fast
    ...
    // Render immediately with "Loading authors…" placeholders
    renderPublications();
    updateIndexStats();
    setOrcidStatus('connected', `${allPublications.length} works`);

    // Step 3: Fetch authors in background via bulk API (batches of 50)
    fetchAuthorsBackground();

  } catch (err) {
    console.error('[ORCID] fetchORCID failed:', err);
    list.innerHTML = `<div class="pub-loading" style="color:var(--red)">
      Could not load publications (${err.message}).
      <a href="https://orcid.org/${ORCID_ID}" target="_blank" style="color:var(--accent);margin-left:0.4rem">View on ORCID ↗</a>
      <br><span style="font-size:0.7rem;color:var(--muted)">Check browser console (F12) for details.</span>
    </div>`;
...
async function fetchAuthorsBackground() {
  // ORCID v3.0 bulk endpoint: /works/{putCode1},{putCode2},...
  // Comma-separated put-codes IN THE PATH — max 100 per request.
  const putCodes = allPublications.map(p => p.putCode);
  const BULK_SIZE = 100;
  ...
  for (let i = 0; i < putCodes.length; i += BULK_SIZE) {
    ...
    try {
      ...
      const res = await fetch(`${ORCID_API}/${ORCID_ID}/works/${path}`, {
        headers: { Accept: 'application/json' }
      });
      if (!res.ok) {
        console.error('[ORCID] bulk fetch failed:', res.status, res.statusText);
        continue;
      }
      const data = await res.json();
      // Bulk response is { bulk: [ { work: {...} }, ... ] }
      ...
      items.forEach(item => {
        ...
        if (!pub) return;
        const contributors = item.work.contributors?.contributor || [];
        ...
        // Patch just the author DOM element — no full re-render needed
      ...
      console.error('[ORCID] bulk author fetch error:', e);
    }
  }
}
