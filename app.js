...
async function fetchAuthorsBackground() {
  // Use ORCID bulk API: fetch up to 50 full records per request
  ...
  for (let i = 0; i < putCodes.length; i += BULK_SIZE) {
    ...
    try {
      const query = chunk.map(c => `put-codes=${encodeURIComponent(c)}`).join('&');
      const res = await fetch(`${ORCID_API}/${ORCID_ID}/works?${query}`, {
        headers: { Accept: 'application/json' }
      });
      if (!res.ok) { debugLog('bulk fetch failed', res.status); continue; }
      ...
      items.forEach(item => {
        ...
        // Update just the author element in the DOM — no full re-render needed
    ...
    } catch (e) { debugLog('bulk author fetch error', e); }
  }
}
